...
class DdbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ddb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ddb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ddb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ddb (date, title, link, content)"
